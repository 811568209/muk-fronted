    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <hr>
                <h5 style="font-weight: 700">收货人信息</h5>
                <div class="col-xs-12">
                    <input type="radio" name="address" id="address1" value="new" v-model=addre>
                    <label for="address1" style="font-weight: normal">
                        <span class="cp-pt">使用新地址</span>
                    </label>
                </div>
                <div v-for="(item,index) in addressList" class="col-xs-12" style="border-bottom: 1px dotted black">
                    <label :for=index style="font-weight: normal;display: block">
                    <input type="radio"  name="address" :id=index :value=item.aid v-model=addre style="zoom:130%">

                        <!--<span class="cp-pt">收货人: 而焦虑     收货地址: 北京市市辖区东城区离开热狗考了两科为了啃啃五年     手机号码: 136254312552</span>-->
                        <span>收货人：<strong>{{item.name}}</strong></span>
                        <span>收货地址：<strong>{{item.shen}}{{item.shi}}{{item.qu}}{{item.address}}</strong></span>
                        <span style="float: right;margin-top: 8px">手机号码：<strong>{{item.phone}}</strong></span>
                    </label>
                </div>
                </div>
                <div class="row cpb" id="addAddress" style="margin-top: 50px;padding-top: 20px">
                    <div class="col-lg-6">
                        姓名
                        <input type="text" class="my-input-cp" v-model="addressDetails.name">
                    </div>
                    <div class="col-lg-6">
                        你的手机号*
                    ...
                    <div class="col-lg-6 cp-flex">
                        <div >
                            省份 *
                            <select name="uaddress" v-model="address.shen" class="cp-option">
                                <option :value=item.value v-for="(item,index) in ssq">{{item.value}}</option>
                            </select>
                        </div>
                        <div >
                            城市 *
                            <select name="uaddress" class="cp-option" v-model="address.shi">
                                <option :value=item.value v-for="(item,index) in ssqshi">{{item.value}}</option>
                            </select>
                        </div>
                        <div >
                            区 *
                            <select name="uaddress" class="cp-option" v-model="address.qu">
                    ...
                    <div class="col-lg-6">
                        固定电话
                        <input type="text" class="my-input-cp" placeholder="例如：0755-123-456" v-model="addressDetails.tel">
                    </div>
                    <div class="col-lg-6">
                        详细地址 *
                        <input type="text" class="my-input-cp" v-model="addressDetails.address">
                    </div>
                    <div class="col-lg-6">
                        邮编
                        <input type="text" class="my-input-cp" placeholder="例如：20000" v-model="addressDetails.mail">
                    </div>
                    <div class="col-xs-12">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <h4>给我们留言</h4>
                        <textarea name="" id="" rows="3" style="width: 100%" v-model="node"></textarea>
                        <hr>
                    </div>
                </div>
                <div class="row  my-boxstyle-c">
                    ...
                    <div class="col-xs-12 my-c-textar"  style="margin-bottom: 40px">
                        <span class="my-button-sc-red cursor" @click="order()">提交订单</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4" style="margin-top: 20px">
                <div class="col-xs-12 cpb">
                    <h4 class="cp-h4">订单总结</h4>
                    <hr>
                    <div class="cp-flex2" style="padding: 0px 15px">
                        <h5>共 <span>{{payList.length}}</span> 件产品</h5>
                        <h5 class="cursor cp-bb" @click="goTo('cart')" v-show="productfromclick.from=='cart'"><< 返回修改</h5>
                    </div>
                    <hr>
                    <div class="col-xs-12 cp-flex2">
                        <h5>
                            商品价格
                        </h5>
                        <h5>
                ...
                <!--订单商品信息-->
                <div class="cp-box2 col-xs-12 cpb bar1 " @mouseover="over()" @mouseout="mout()" id="myscrollbar">
                    <h4 >商品信息</h4>
                    <div class=" clear" style="" v-for="item in payList">
                        <hr>
                        <div style="width: 72px;float: left" @click="goTo('/productDetails/',item.pid)">
                            <img :src=item.show_img alt="" class="img-responsive cursor">
                        </div>
                        <div style="float: left">
                            <p>{{item.title2}}</p>
                            <p>{{item.selected==1 ? item.select1 : item.selected==2 ? item.select2 :  item.selected==3 ? item.select3 : item.select1 }}</p>
                            <p>数量 {{item.count}}</p>
                        </div>
                        <div style="float: right;margin-top: 55px">
                            {{item.selected==1 ? item.price1 : item.selected==2 ? item.price2 :  item.selected==3 ? item.price3 : item.price1 | currency}}
                        </div>
                    </div>
                </div>
            </div>
    ...
    export default{
        data(){
            return {
                ...
                ssqqu:[],
                address:{shen:'',shi:'',qu:''},
                addressDetails:{name:'',address:'',phone:'',tel:'',mail:'',shen:'',shi:'',qu:'',uid:0},
                addre:'new',
                productfromclick:{},
                payList:[],
                cost:0,
        ...
        mounted: function () {
            ...
            this.address.shen='北京市';
            this.productfromclick=this.$route.params
//            console.log(this.user);
            console.log(this.productfromclick);
            this.getList()
            this.getAddressList()
//            setInterval(()=>{
...
//                    console.log('address');
                for (let elem of this.ssq){
                    if (elem.value ==this.address.shen){//查找省数组中与选择的省份匹配的值
//                            console.log(elem);
...
//                            console.log(this.ssqshi);
                        ...
                        for (let elem of this.ssqshi){//查找用户选择的市地址与新生成市列表中有没有匹配项
                            if (this.address.shi==elem.value){
                                include=true//如果存在匹配标记为true
                            }
                        }
//                        console.log(include);
                        if (include==false){//如果不存在匹配说明市列表是新生成的，把用户选择市地址赋值为市列表的第一个
                            this.address.shi = this.ssqshi[0].value
                        }
                    }
                }
                for (let elem of this.ssqshi){
                    if (elem.value ==this.address.shi){//查找省数组中与选择的市匹配的值
                        this.ssqqu=elem.children//把市数组中匹配的子数组赋值给区数组
                        let include=false
                        for (let elem of this.ssqqu){//查找用户选择的区地址与新生成区列表中有没有匹配项
                            if (this.address.qu==elem.value){
                                include=true//如果存在匹配标记为true
                            }
                        }
//                        console.log(include);
                        if (include==false){//如果不存在匹配说明区列表是新生成的，把用户选择区地址赋值为区列表的第一个
                            this.address.qu = this.ssqqu[0].value
                        }
                    }
                }
            },
            over:function (e) {
                let myscrollbar=document.querySelector("#myscrollbar");
                myscrollbar.className='col-xs-12 cpb bar2 cp-box2'
            },
            mout:function () {
                let myscrollbar=document.querySelector("#myscrollbar");
                myscrollbar.className='col-xs-12 cpb bar1 cp-box2'
            },
            getList:function () {
                let pid=''
                if (this.productfromclick.from=='productDetails'){
                    pid = this.productfromclick.data.pid
                    this.$http.post(this.urlAdree+'/order/list/pd',{pid:pid},{emulateJSON:true}).then((response)=>{
                        console.log(response.body);
                        this.payList=response.body
                        this.payList[0].count=this.productfromclick.data.count
                        this.payList[0].selected=this.productfromclick.data.selected
                        this.totalCost()
                    })
                }else if (this.productfromclick.from=='cart'){
                    let cid = this.productfromclick.data.stringCid.split('::')
                    cid.shift()
                    cid.unshift(this.user.userid)
                    cid=cid.join('::')
                    this.$http.post(this.urlAdree+'/order/list/ca',{cid:cid},{emulateJSON:true}).then((response)=>{
                        console.log(response.body);
                        this.payList=response.body
                        this.totalCost()
                    })
            ...
            totalCost:function () {
                ...
                for(let index=0;index<this.payList.length;index++){
                    total_prices+=this.payList[index].count*(this.payList[index].selected==1 ? this.payList[index].price1 : this.payList[index].selected==2 ? this.payList[index].price2 : this.payList[index].price3);
                }
                this.cost=total_prices
            },
            order:function () {
                if (this.payList.length<='0'){
                    this.mesbox('请返回选择商品',5000)
                    return
                }
                if (this.productfromclick.from=='productDetails'){
                    if (this.addre=='new'){
                        this.mesbox("请选择一个地址！",4000)
                        return
                    }
                    let pid = this.payList[0].pid;
                    let total_prices=this.payList[0].count*(this.payList[0].selected==1 ? this.payList[0].price1 : this.payList[0].selected==2 ? this.payList[0].price2 : this.payList[0].price3);
                    this.$http.post(this.urlAdree+'/order/purchase/pd',{uid:this.user.userid,pid:pid,selected:this.payList[0].selected,count:this.payList[0].count,total_prices:total_prices,cost:this.cost,aid:this.addre,node:this.node},{emulateJSON:true}).then((response)=>{
                        console.log(response.body);
//                        this.$router.push('/')
                        setTimeout(()=>{
                            this.$router.push({
                    ...
                    let cid = []
                    for (let elem of this.payList){
                        cid.push(elem.id)
                    }
                    cid=cid.join('::')
                    this.$http.post(this.urlAdree+'/order/purchase/ca',{uid:this.user.userid,cid:cid,cost:this.cost,aid:this.addre,node:this.node},{emulateJSON:true}).then((response)=>{
                        console.log(response.body);
//                        this.$router.push('/')
                        setTimeout(()=>{
                            this.$router.push({
            ...
            addressSave:function () {
                ...
                let addresspuls=this.address.shen+this.address.shi+this.address.qu+this.addressDetails.address
//                console.log(addresspuls);
                ...
                this.addressDetails.shen=this.address.shen
                this.addressDetails.shi=this.address.shi
                this.addressDetails.qu=this.address.qu
                this.addressDetails.uid=this.user.userid
                this.$http.post(this.urlAdree+'/address/add',this.addressDetails,{emulateJSON:true}).then((response)=>{
                    console.log(response.body);
                    this.mesbox(response.body.mesg,3000);
                    this.getAddressList()
                    this.addre=response.body.aid;
...
//                console.log(/^\d+$/.test(this.addre));
                if (/^\d+$/.test(this.addre)){
...
<style scoped>
    .cp-bb:hover{border-bottom: 1px solid black;font-weight: 600}
    .cp-box2{height: 500px;overflow-x: auto;margin-top: 20px}
    .cp-box2>div{color: #747474;margin-top: 10px}
    .cp-box2>h4{text-align: center;font-weight: 600}

    .cp-flex2 {display: flex;justify-content: space-between}
    .cp-h4{
    ...
    .cp-pt {position: relative;top: -2px}
    #address1{
        zoom: 130%
    }
    #address2{
    ...
    html,body {
        scrollbar-face-color:rgba(0,0,0,0.3);; /*滚动条3D表面（ThreedFace）的颜色*/
    ...
    .bar1::-webkit-scrollbar-track:hover
    {
        -webkit-box-shadow: inset 0 0 6px rgba(0,0,0,0.3);
        border-radius: 10px;  /*滚动条的背景区域的圆角*/
        background-color: snow;;/*滚动条的背景颜色*/
    }
    .bar2::-webkit-scrollbar-track:hover
    {
        -webkit-box-shadow: inset 0 0 6px rgba(0,0,0,0.3);
        border-radius: 10px;  /*滚动条的背景区域的圆角*/
        background-color: snow;;/*滚动条的背景颜色*/
    ...
    .bar1::-webkit-scrollbar-thumb
    {
        ...
        background-color: rgba(250,250,250,.3);;  /*滚动条的背景颜色*/
    ...
    .bar2::-webkit-scrollbar-thumb
    {
        ...
        background-color: silver;  /*滚动条的背景颜色*/
    }
    .bar1::-webkit-scrollbar-thumb:hover
    {
        border-radius: 10px;  /*滚动条的圆角*/
        -webkit-box-shadow: inset 0 0 6px rgba(0,0,0,.3);
        background-color:rgba(0,0,0,0.6);  /*滚动条的背景颜色*/
    }
